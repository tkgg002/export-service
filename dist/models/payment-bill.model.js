...
class PaymentBillModel extends Model {
    ...
    static async findWithPayments(filter = {}, sort = { createdAt: -1 }, skip = 0, limit = 1000) {
        const where = { is_delete: false };
        if (filter.createdAt && (filter.createdAt.$gte || filter.createdAt.$lte)) {
            where.created_at = {};
            if (filter.createdAt.$gte)
                where.created_at[Op.gte] = filter.createdAt.$gte;
            if (filter.createdAt.$lte)
                where.created_at[Op.lte] = filter.createdAt.$lte;
        }
        if (filter.merchantId)
            where.merchant_id = filter.merchantId;
        if (filter.merchantEmail)
            where.merchant_email = filter.merchantEmail;
        if (filter.state)
            where.state = filter.state;
        if (filter.channelID)
            where.channel_id = filter.channelID;
        if (filter.apiType)
            where.api_type = filter.apiType;
        if (filter.orderId)
            where.order_id = filter.orderId;
        if (filter.merchantTransId)
            where.merchant_trans_id = filter.merchantTransId;
        if (filter.partnerCode)
            where.partner_code = filter.partnerCode;
        if (filter.trackingId)
            where.tracking_id = filter.trackingId;
        if (filter.lastUpdatedAt && (filter.lastUpdatedAt.$gte || filter.lastUpdatedAt.$lte)) {
            where.updated_at = {};
            if (filter.lastUpdatedAt.$gte)
                where.updated_at[Op.gte] = filter.lastUpdatedAt.$gte;
            if (filter.lastUpdatedAt.$lte)
                where.updated_at[Op.lte] = filter.lastUpdatedAt.$lte;
        }
        return await PaymentBillModel.findAll({ where, offset: skip, limit, raw: true });
    }
}
export default PaymentBillModel;
//# sourceMappingURL=payment-bill.model.js.map