...
config({ path: '.run.local.env' });
console.log("MONGO_URI from process.env:", process.env.MONGO_URI);

import { ServiceBroker, Service, Context } from "moleculer";
import brokerConfig from "./configs/moleculer.config.ts";

import { v4 as uuidv4 } from "uuid";
import MongoDb from "./dbHandler/mongo.ts";
import * as modelsIndex from "./models/index.ts";
...
import MainExportService from "./src/services/export-service.ts";
import ExportWorker from "./src/workers/export.worker.ts";
...
broker.createService(ExportServiceSchema);
broker.start();